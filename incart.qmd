...
```{r}
#| label: parse patient-record info
info_file <- 'files-patients-diagnoses.txt'
info <- readLines(file.path(data_location, info_file))
patient_files <- tibble(id = gl(32, 3), txt = info) %>%
  summarize(pid = txt[1],
            files = txt[2],
            .by = id) %>%
  separate_longer_delim(cols = files, delim = ' ') %>%
  select(-id) %>%
  rename(id = files)

headers <- right_join(headers, patient_files, by = 'id') %>%
  select(-header_file, -id) %>%
  distinct()
knitr::kable(headers, digits = 1)
```

```{r}
#| label: count NA diagnosis 
headers %>% count(diagnoses) %>%
  knitr::kable()
```

There are a considerable number of patients in the database that have provided ECG but for which no diagnosis is provided. It maybe that these are healthy patients or the diagnosis may have been omitted for other reasons.
...
To calculate disparate impact and statistical parity (ignoring the point whether this makes sense), for the Python AIF360 package, we need to have both presence and absence of diagnoses for each patient. **Note that the patients without diagnosis will as such only have absence scores** and this may result in artificial bias.
...
dat <- headers %>%
  ...
  pivot_longer(cols = -c(pid, age, gender, age_bin), names_to = 'diagnosis',
               values_to = 'present') %>%
  filter(diagnosis != 'NA') %>%
...
```{r}
#| label: age_bins
headers %>% distinct(pid, age_bin) %>% count(age_bin) %>% knitr::kable()
```
